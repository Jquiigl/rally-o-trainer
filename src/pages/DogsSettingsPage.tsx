...
export function DogsSettingsPage() {
  ...
  const activeSession = useLiveData(() => db.sessions.where('status').anyOf('active', 'paused').first(), [], undefined);
  ...
  return <>
    ...
    <section className="card legal-note"><h2>Información y autoría</h2><p>Rally O Trainer es una herramienta independiente basada en fuentes RSCE/FCI. Sus explicaciones son redacción propia y no sustituyen los reglamentos oficiales.</p><Link className="button button--ghost" to="/authorship">Autoría y propiedad intelectual</Link></section>
