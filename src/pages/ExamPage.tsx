...
export function ExamPage() {
  ...
  return <>
    ...
    <section className="card exam-card"><p className="eyebrow">¿Qué señal describe?</p><h1>{question.prompt}</h1><div className="answer-list">{question.options.map((signalId) => { const signal = getSignalSafe(signalId); const state = answered ? signalId === question.signalId ? 'correct' : signalId === answered ? 'wrong' : '' : ''; return <button key={signalId} className={state} disabled={Boolean(answered)} onClick={() => answer(signalId)}><span>{signal.officialNumber}</span>{signal.name}</button>; })}</div>{answered && <div className={`feedback ${correct ? 'correct' : 'wrong'}`} role="status"><strong>{correct ? 'Correcto' : 'La respuesta correcta es otra.'}</strong><p>{signals.find((item) => item.id === question.signalId)?.regulatoryDescription}</p><button className="button button--primary" onClick={() => { setAnswered(null); setIndex((value) => value + 1); }}>{index === questions.length - 1 ? 'Ver resultado' : 'Siguiente'}</button></div>}</section>
  </>;
}
